...
[ -n "$sevenzip" ] || {
    echo "error: 7z/7za not found (apt install p7zip-full)" >&2
    exit 1
}
command -v unshield >/dev/null 2>&1 || {
    echo "error: unshield not found (apt install unshield)" >&2
    exit 1
}
...
[ -n "$CAB" ] && [ -f "$(dirname "$CAB")/data1.hdr" ] || {
    echo "error: data1.cab / data1.hdr not found in archive" >&2
    exit 1
}
...
for g in "Distiller Program Files" "Distiller Win NT System files" \
    ...
    unshield -g "$g" -d "$TMP/$g" x "$CAB" >/dev/null || {
        echo "error: could not extract group '$g' (unexpected cabinet contents)" >&2
        exit 1
    }
done
...
[ -n "$pf" ] || {
    echo "error: acrodist.exe not present in cabinet" >&2
    exit 1
}
cp -a "$pf/." "$D/"
...
[ -n "$psd" ] || {
    echo "error: psdisk resource tree not present in cabinet" >&2
    exit 1
}
cp -a "$psd" "$D/Data/"
...
[ -f "$D/acrodist.exe" ] || {
    echo "error: assembly incomplete (no acrodist.exe)" >&2
    exit 1
}
